...
class Settings(BaseSettings):
    ...
    def validate_production(self) -> "Settings":
        ...
        if len(self.api_key) < 32 or "change-me" in self.api_key.lower():
            raise ValueError("production RAEBURN_API_KEY must be a unique value of at least 32 characters")
        ...
        if self.github_writeback_enabled and (not self.github_token or not self.github_default_repository):
            raise ValueError("GitHub writeback requires GITHUB_TOKEN and GITHUB_DEFAULT_REPOSITORY")
        if self.jira_writeback_enabled and not all(
            [self.jira_base_url, self.jira_email, self.jira_api_token, self.jira_project_key]
        ):
            raise ValueError("Jira writeback requires base URL, email, API token and project key")
        ...
        if self.webhook_writeback_enabled:
            if not self.webhook_url or not self.webhook_url.startswith("https://"):
                raise ValueError("production webhook writeback requires an HTTPS WEBHOOK_URL")
