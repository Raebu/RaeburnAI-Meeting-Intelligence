...
def test_production_rejects_email_without_starttls() -> None:
    with pytest.raises(ValidationError, match="must require STARTTLS"):
        production_settings(
...
def test_production_accepts_workspace_scoped_writeback_configuration() -> None:
    ...
    assert settings.email_followup_enabled is True
    assert (
